        ...
        <tbody>
          <?php foreach ($fallos as $f) : ?>
            <tr id="idCort<?= $f->id_maquila_corte ?>">
              ...
              <td class="d-md-table-cell d-none cantidad" name="cantidad"><?= htmlentities($f->Cantidad) ?></td>
              <td class="d-md-table-cell d-none engregadas" name="engregadas"><?= htmlentities($f->Fecha_enrtrega) ?></td>
              <td class="d-md-table-cell d-none"><?= htmlentities($f->name) ?></td>
              <td>
...
<?php $this->load->view("sscm/AsignacionMaquilas/RevisionModal.php") ?>
<script src="<?php echo base_url('index.php/../template/js/revision.js') ?>"></script>